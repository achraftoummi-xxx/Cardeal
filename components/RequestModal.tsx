...
import { X, CheckCircle2, AlertCircle, Loader2, CalendarDays, FileText } from "lucide-react";
...
export type RequestMode = "appointment" | "quote";
...
export default function RequestModal({ partner, mode, onClose }: Props) {
  ...
  const buildPayload = (): PendingRequest => ({
    type: mode,
    partnerId: partner.id,
    partnerName: partner.name,
    fullName: form.fullName.trim(),
    phone: form.phone.replace(/\D/g, ""),
    date: isAppointment ? form.date : undefined,
    time: isAppointment ? form.time : undefined,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    /* Unauthenticated: persist the request and go sign up.
       After signup the app replays it and confirms delivery. */
    if (!isAuthenticated()) {
  ...
  const title = isAppointment ? t("appointment.title") : t("requestModal.quoteTitle");
  const subtitle = isAppointment
    ? t("appointment.subtitle", { partner: partner.name })
    : t("requestModal.quoteSubtitle", { partner: partner.name });
      ...
      <div className="relative mx-4 flex min-w-0 max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
            ...
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <h3 className="text-base font-bold">
                {isAppointment
                  ...
                  : t("requestModal.quoteSuccessTitle")}
              </h3>
              <p className="mt-2 text-sm text-muted-foreground">
                {isAppointment
                  ? t("appointment.successMessage", { partner: partner.name })
                  : t("requestModal.quoteSuccessMessage", { partner: partner.name })}
              </p>
            </div>
          ) : (
            <form id="request-form" onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label htmlFor="request-phone" className={labelClasses}>
                  {t("appointment.phone")}
                </label>
                <input
                  id="request-phone"
        ...
        <div className="shrink-0 border-t border-border bg-card/95 px-4 py-4 backdrop-blur-xl sm:px-6">
          {status === "success" ? (
            ...
            <Button type="submit" form="request-form" className="w-full" disabled={status === "submitting"}>
              {status === "submitting" ? (
                <>
                  <Loader2 size={15} className="animate-spin" />
                  {t("requestModal.sending")}
                </>
              ) : (
                <>
                  {isAppointment ? <CalendarDays size={15} /> : <FileText size={15} />}
                  {t("requestModal.send")}
                </>
              )}
            </Button>
