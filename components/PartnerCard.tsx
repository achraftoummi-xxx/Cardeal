...
export default function PartnerCard({ partner, origin, active = false, onSelect, onBook, onQuote }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex w-full min-w-0 flex-col gap-2 border-t border-border pt-3 sm:flex-row">
        <Button
          onClick={(e) => {
            e.stopPropagation();
            onBook?.();
          }}
          className="flex w-full min-w-0 max-w-full items-center justify-center gap-2"
        >
