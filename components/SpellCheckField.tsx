...
export function SpellCheckInput({ value, onChange, className = "", ...props }: SpellCheckInputProps) {
  ...
  // Check text spelling on value change (debounced or on word boundaries)
  useEffect(() => {
  ...
  const handleApplySuggestion = (errIdx: number, suggestion: string) => {
    ...
    const newWords = words.map((w, idx) => {
      if (w.trim().length > 0) {
        if (targetCount === errIdx) {
          // preserve leading/trailing punctuation or casing if desired
          return suggestion;
        }
        targetCount++;
      }
    ...
    // Trigger synthetic change event for parent forms
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <input
        ...
        className={`${className} ${hasErrors ? "border-amber-500/80 focus:ring-amber-500/30" : ""}`}
      />
        ...
        <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
          {errors.map((err, idx) => (
            <div key={idx} className="relative">
              ...
              {activeDropdown === idx && (
                <div className="absolute right-0 bottom-full mb-2 z-50 w-48 rounded-xl border border-zinc-700 bg-zinc-900/95 p-2 shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95">
                  <div className="px-2 py-1 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider border-b border-zinc-800 mb-1">
                    Suggestions for &quot;{err.word}&quot;
                  </div>
                  <div className="flex flex-col gap-1">
                    {err.suggestions.map((sug, sIdx) => (
                      ...
                      >
                        <span>{sug}</span>
                        <span className="text-[10px] text-zinc-500">replace</span>
                      </button>
                    ))}
                  </div>
                </div>
