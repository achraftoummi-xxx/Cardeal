...
export default function AdminRootLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background px-4 text-foreground">
        <div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center shadow-xl">
          <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[var(--cardeal-primary)]/10 ring-1 ring-[var(--cardeal-primary)]/30">
            <AlertCircle size={26} className="text-[var(--cardeal-primary)]" />
          </span>
          <h1 className="mt-4 text-lg font-bold text-foreground">Accès Restreint Admin</h1>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans antialiased">
      {/* Top Admin Header */}
      <header className="sticky top-0 z-40 border-b border-border bg-card/90 px-4 py-3 sm:px-6 backdrop-blur-xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            href="/dashboard"
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-secondary text-foreground hover:bg-accent transition"
            title="Quitter l'Admin"
          >
            <ArrowLeft size={18} />
          </Link>
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--cardeal-primary)]/20 border border-[var(--cardeal-primary)]/40 text-[var(--cardeal-primary)]">
              <ShieldCheck size={20} />
            </div>
            <div>
              ...
              <p className="text-[11px] text-muted-foreground hidden sm:block font-['Manrope']">Command & Operations Center</p>
            </div>
          </div>
        </div>
      ...
      <nav className="border-b border-border bg-card/50 px-4 sm:px-6 overflow-x-auto">
      ...
      {/* Main Content Area */}
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 font-['Manrope']">
        {children}
      </main>
    </div>
  );
